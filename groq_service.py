...
🎯 JSON: ["mix", "cat2", "cat3", "cat4"]"""
    ...
    async def generate_dishes_list(products: str, category: str) -> List[Dict[str, str]]:
        ...
        # Определяем язык ввода для корректного отображения названий
        ...
        if category == "mix":
            # Для mix используем фиксированные названия на русском, но с правилом языка
            if input_language == "ru":
                name_template = "Суп"
            else:
                name_template = "Soup (Суп)"
...
- Если продукты на другом языке: используй названия на языке оригинала без перевода на русский язык: "Soup (Суп)"

🎯 ТРЕБОВАНИЯ К МЕНЮ:
- СТРОГО 4 блюда в списке: 1) Суп/Soup, 2) Второе блюдо/Main course, 3) Салат/Salad, 4) Напиток/Drink
...
[
  ...
  {{ "name": "{'Напиток' if input_language == 'ru' else 'Drink (Напиток)'}", "desc": "Аппетитное описание на русском" }}
            ...
            # Для обычных категорий сохраняем язык ввода в названиях
            language_rule = ""
            if input_language == "ru":
...
- Поле "desc": Описание на русском языке"""
                ...
                language_rule = """🎯 ПРАВИЛА ЯЗЫКА:
- Поле "name": Название блюда НА ЯЗЫКЕ ВВОДА (оригинале) + перевод в скобках если нужно (пример: "Pasta Carbonara (Паста Карбонара)")
- Поле "desc": Описание на РУССКОМ ЯЗЫКЕ"""
...
🎯 JSON: [{{ "name": "...", "desc": "..." }}]"""
        ...
        try:
            ...
            # Для mix категории проверяем структуру
            if category == "mix":
                if len(dishes) != 4:
                    # Если пришло не 4 блюда, создаём правильную структуру
                    expected_names = [
                    ...
                    # Если пришли какие-то блюда, используем их описания, иначе создаём новые
            ...
            logger.error(f"Ошибка парсинга JSON в generate_dishes_list: {e}")
            return []
...
"""
    ...
    async def generate_freestyle_recipe(dish_name: str) -> str:
        ...
        # Определяем язык ввода по названию блюда
        input_language = GroqService._detect_input_language(safe_dish_name)
        
        if input_language == "ru":
            translation_rule = "Пиши названия ингредиентов СТРОГО на русском языке без скобок и повторений."
        else:
            translation_rule = "Название блюда и ингредиенты — на языке оригинала, перевод на русский в скобках."
